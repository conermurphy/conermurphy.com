...
they unfortuantely can't help with the getting visitors to your website part. But, this is where the Open Graph protocol and more importantly Open Graph Images come into play. In this post, I'm going
to cover:
...
When you set these tags on one of your website's blog posts or pages, it provides the content that will be used to popualte the 'card' shown to users on social media sites when they link to 
...
The first thing we need to do is add an SEO Component to our website. There's a good guide on making one with an example SEO Component on Gatsbys website
...
The final step for us to add Open Graph support to our blog posts is modiyfing our blog post template file which for me is *blogPost.js* but this is whatever file is responsible for creating
...
posts.forEach(({ node }, index) => {
    createPage({
      ...
      component: path.resolve('./src/templates/blogPost.js'), // this file.
      context: {
...
What we're doing here is assigning the variable post to the object returned from data.mdx and then in a seperate variable destructing out the image object into it's own variable from the frontmatter.
...
we assigned to the *image* varaible.
...
In this case, I have imported logo seperately and just referenced it here. But now should we not set an image or it be undefined for whatever reason it will default to whatever we set on the right
