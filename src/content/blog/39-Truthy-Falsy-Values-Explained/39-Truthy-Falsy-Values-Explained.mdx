---
title: "Truthy and Falsy Values Explained"
date: "2021-05-22"
tags: ["JavaScript"]
slug: truthy-falsy-values-explained
image: ./truthy-falsy-values-explained.png
published: true
canonical_url: ""
description: ""
---

A large part of using JavaScript is dealing with booleans. This might be in conditional rendering or just a simple if statement. But, regardles booleans are everywhere.
...
This means that any time we create a value, wether it be a function, an object or just a simple variable decleration it will evaluate to true.
...
Because `helloWorld` has a value it is `truthy` and will satify the if condition and run the code block.

Here is an example of values that can be considered to be `truthy`.

![List of Truthy Values](./truthy-values.png)
...
![List of Falsy Values](./falsy-values.png)

_Read more about [[[IsHTMLDDA]]](https://tc39.es/ecma262/#sec-IsHTMLDDA-internal-slot)_

If we was to use one of these in the if statement from before it wouldn't run the code block.
...
Because `helloWorld` is now an empty string (`falsy`) it won't satify the if statement condition and will skip the code block.
...
Over time dealing with `truthy` and `falsy` values becomes second nature but at first it can through you into a spin where you can't figure out why a variable is evaluating to false.
