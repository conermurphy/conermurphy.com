...
We all know the benefits that building a following online can bring. One of the most powerful tools for someone looking to build a following online is an email newsletter.
...
It expands to include new values if required, all we need to do is pass in new defaults.
...
First, we destructure out the `values` and the `updateValue` function that we returned. Then we destructure out the individual values on the next line.
...
Here you can see that we destructure out the value we want to set to state from the event with `e.target`. Then when we set to state we get the `name` of the input from `e.target` again to be the key.
...
At this point the function hasn't called, so we create the state has accordingly:
...
This gives us access to all the state we defined in the function which allows us to display it. It also gives us access to the `submitEmail` function we defined.
...
The one thing we could add to improve this setup would be a Honeypot to capture any robots trying to fill in the form. But, I plan on covering this in a separate post where I can go more in-depth on them.
