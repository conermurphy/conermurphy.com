...
HTTP status codes are a vital part of the web, they allow for clear communication between clients and servers. A code indicates if everything is okay, if there's been an error and where, or that it's still in progress.

For us developers, they can give a great insight into where problems may be occurring. But, did you know that certain status codes can have an impact on your SEO?

In this post, I'm going to cover what HTTP status codes are, the different categories and how certain ones can influence your SEO. Let's get started.
...
Google crawls websites every day and yours is no exception. They check the status codes of URLs compared to the previous time they crawled your website.
...
Contrary to what has believed it is actually bad practice to redirect every missing page to your home page or another with a 301.
...
When Google receives a 404 for a URL it will actually stop indexing that page and crawling in the future.
...
As I alluded to earlier there is an important reason to use a 301, that is that the Link Equity is redirected with the URL.
...
As mentioned above there are sometimes where you might want to set a 404 response. But, it can be hard to tell when to redirect and when to let it 404.
...
Similar to a 404, 410 responses are no longer indexed by Google once it is received.
...
Because a 503 could be caused by server maintenance or the server being temporarily offline. When a search engine crawler hits a 503 response it will return later on and try to crawl it again.
