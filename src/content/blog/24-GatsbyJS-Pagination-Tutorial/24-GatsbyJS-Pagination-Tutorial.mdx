...
image: ./GatsbyJS-Pagination-Tutorial.png
id: 24
---
...
_An important thing to note is that if you want your first page to show the latest blog post you need to use the `sort` property in the GraphQL query to ensure the first item is the latest one. See my example above for how to sort using the frontmatter\_\_\_date field._

## Pagination Controls

It's great having our posts paginated out over several pages. But, if the only way we have to move between them is changing the URL we're going to have issues.

This is where adding pagination controls comes in, we're going to add a couple of dedicated buttons that allow next and previous movement on pages. We will also add in a list of pages in the middle of the buttons so you can skip pages if you wish.

### Creating the Pagination Component

Before we can set about implementing it into our blog page, we need to actually create our `Pagination` component. Let's take a look at the code now.
...
Overall, the code isn't to complex, we have a couple of `Link` elements from Gatsby coupled with another one of `Array.from()` specials from earlier. This time though instead of using a `.forEach()` we use a `.map()` because we want to return the elements to display.

Let's break this down into 3 segments.
...
1. pageSize: The amount of posts we want on each page.
2. totalCount: The total amount of blog pages there are from the GraphQL query on the blog page
3. currentPage: This comes from the context we added in `gatsby-node.js`, it is destructured out in the blog page and then passed down to this component.

After this we create a series of variables that will be used to determine if there is another page to navigate to and what the URL would be for that page.

1. totalPages: Int: The total amount of pages required
2. prevPage: Int: Works out the index of the previous page based off the current one.
3. nextPage: Int: Works out the index of the next page based off the current one.
4. hasNextPage: Bool: Checks if the `nextPage` variable has an index value higher smaller than or equal to the `totalPages`.
5. hasPrevPage: Bool: Checks if the `prevPage` variable is greater than or equal to one.

These 5 variables are then used throughout the rest of the component to decide whether elements should be displayed or not but we'll come to that in a second. First we need to create the next and previous links which are our final two variables.
...
2. nextLink: Str: This is just set to `blog/${nextPage}`. This can be done because we have a seperate variable controlling whether there is a another page after or not.

Now we've convered all of the variables required to create the elements, let's look at making them.

#### Next and Previous buttons

The code for the previous and next buttons is actually quite similar with just a few word changes so I'm just going to break one of them down so we can understand how it works but here is both of them:
...
The general structure of the links is a Gatsby `Link` element wrapping a `span` declaring what the button does to the user. Let's check out what we do with them:

1. title: Str: We set a title attribute on the element to declare to the user what each link does.
2. disabled: Bool: This is where the two boolean variables `hasPrevPage` and `hasNextPage` come in from earlier. If the boolean says that the next or previous page is outside the available pages it will be false and disable the link here. This is then styled by CSS to lower the opacity and prevent the user clicking on them. Check below for this code.
3. to: Str: This is the destination we want the element to navigate to if clicked.

This breaks down the functionality of the links, the last part is adding the text elements in to label them up on screen. For this we just use a `span` wrapping the revelant word and a next / previous arrow depending on the button.
...
We're almost there with the pagination component, just need to add in the links for each page between the previous and next buttons.
...
It's based off a similar principle to what we used in the past for generating each page in `gatsby-node.js`. However, instead of using a `.forEach()` to loop over each index, we use a `.map()` so we can return the `Link` element.
...
4. className: Str: Used to add the 'current' class to the element if we are on that page to allow for styling to distinguish it.
...
As we have split the entire pagination component, it's now just a matter of passing in the required props:
...
We should now be setup and ready to rock and roll.

## Summing Up

If everything has gone to plan you should now having something that resembles this in functionality:

```
insert picture of final pagination and blog page
```

I didn't include the CSS in this article apart from one or two sections to try and keep the length of the post down as it's already a long one. But, if you are interested all of the CSS I use is available on the [Github for my website.](https://github.com/conermurphy/conermurphy.com)

One thing I didn't include in this post but do recommened is storing your `pageSize` variable in a `.env` file and importing it into Gatsby using `parseInt(process.env.GATSBY_PAGE_SIZE)}` for example. This means that should you ever want to change the amount of posts per page you only need to change it in one place rather than multiple.

I hope you found this post helpful if you did I would greatly appreciate you sharing it with others on Social Media so they can benefit from it to. If you have any feedback I would love to hear it, you can [reach me over on Twitter.](https://twitter.com/MrConerMurphy)
