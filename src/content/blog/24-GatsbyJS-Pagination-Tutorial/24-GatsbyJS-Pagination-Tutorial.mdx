...
2. The number of pages being determined by the number of posts we want per page.
3. Finally, we add controls to navigate between pages with the ability to skip to a certain page.

### What Are We Creating?

Below is a screenshot from my live website [conermurphy.com](). Today, we will be covering the functionality that allows us to specify I want X posts per page and then have Gatsby create the pages for us. For the purposes of this tutorial, I will use a blog as an example, but this logic can apply to any page on a website. For example, I also use it with my notes on the blog.
...
![Blog Page of conermurphy.com](./Conermurphy-Blog-Page.png)
...
_In the following examples of code, I use ES6 Modules in my gatsby-node file, these aren't enabled by default. [You can follow a tutorial I did on Twitter to set these up.]()_
...
Anyway, back on topic. Next, we need to do is create the `turnBlogPostsIntoPages` function that we declared.
...
The most important part of this is what happens on line 1. This is the part that enables us to create multiple blog pages.
...
Now, we are able to loop through an array of variable length to allow us to create the required number of pages. Now we need to do the actual creation.
...
Now, we have the `gatsby-node.js` file setup and ready to create all the pages for us. We need to amend the page file that lists all the posts for us.
...
_If you want your first page to show the latest post you need to use the `sort` property in the query. See my example above for how to sort using the frontmatter\_\_\_date field._
...
The code isn't too complex, we have a couple of `Link` elements from Gatsby. Then another one of `Array.from()` specials from earlier. This time though instead of using a `.forEach()` we use a `.map()` because we want to return the elements.
...
After this, we create a series of variables that will are used to determine if there is another page to navigate to. And, if so what the URL is.
...
Now we've covered all the variables required to create the elements, let's look at making them.

#### Next and Previous Buttons

The code for the previous and next buttons is actually similar with a few word changes. For this reason, I'm only going to break one of them down so we can understand how they both work. First let's look at the complete code:
...
![Blog Page of conermurphy.com](./24-Conermurphy-Blog-Pagination.gif)
