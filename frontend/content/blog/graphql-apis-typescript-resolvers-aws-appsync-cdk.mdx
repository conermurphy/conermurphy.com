---
UUID: "e5c9387d-b10b-40c3-ae11-94585c8c81d0"
title: "Building GraphQL APIs With TypeScript Resolvers Using AWS AppSync and CDK"
description: "Learn to create powerful and scalable GraphQL APIs using AWS AppSync with TypeScript resolvers via the AWS CDK."
date: "2023-10-19"
topics: ["AWS", "APPSYNC", "DYNAMODB"]
slug: graphql-apis-typescript-resolvers-aws-appsync-cdk
image:
published: true
canonical_url: ""
imageLink:
...
Because GraphQL is fully typed via its schema we’re able to generate TypeScript types from it automatically. To do this, we’re going to use the packages `@graphql-codegen/cli`, `@graphql-codegen/typescript`, and `@graphql-codegen/typescript-resolvers` which we can install our project by running `npm install -D @graphql-codegen/cli @graphql-codegen/typescript @graphql-codegen/typescript-resolvers`.

With those packages installed, create a new file called `.graphqlconfig.yml` inside the `graphql` directory alongside our `schema.graphql` file from earlier. Then inside that file, add the below code.

```yaml:[]:./graphql/.graphqlconfig.yml:YAML
...
With this config file, we point the `graphql-codegen` command we’re going to run in a moment to our schema file as well as configure the output file (`./types/graphql.ts`) which will contain all of our generated TypeScript types for us.

Now with the configuration file setup and packages installed, make sure you’re in the `graphql` directory in your terminal and then run the command `npx graphql-codegen --config .graphqlconfig.yml`. Once this command finishes all of your types should be created and available in `./types/graphql.ts`.
