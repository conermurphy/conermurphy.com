...
Just a quick note on arrayLike values, without going to in depth on them as we covered them more extensively in [#4](/blog/javascript-array-prototype-methods-from-explained/). arrayLike values are values that share some methods with arrays and are visually shown to be the same syntax as an array but they are a different data type to a full array which is why sometimes you need to convert an arrayLike value to an array using Array.from() that we looked at in [#4](/blog/javascript-array-prototype-methods-from-explained/).
