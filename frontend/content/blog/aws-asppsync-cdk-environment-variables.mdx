---
UUID: "0a5fa93b-ef20-44eb-9b10-5eecda9e7ed4"
...
_NOTE:_ In this case I’ve set the API key to expire on 5th March 2025. But, if you’re reading this tutorial after that date or want to experiment with different values. [Use this website to generate a new Epoch timestamp.](https://www.epochconverter.com/)
...
```ts:[]:./graphql/resolvers/ts/create-item.ts:TYPESCRIPT
...
```ts:[]:./graphql/resolvers/ts/get-item.ts:TYPESCRIPT
import { Context } from "@aws-appsync/utils";
...
You should receive a response like the one from the API.
...
Finally, let’s test everything was stored in the database correctly. So, switch out your mutation query for the query below and make sure to add in the `id` from the item you just created.
...
```json
{
  "data": {
    "getItem": {
      "id": "9c3960ee-3137-4455-9a51-83463d3ef338",
      "value": "some-secret-value"
    }
  }
}
